...
(function($){
	$.fn.extend({ 
		...
		var randID =  Math.round(Math.random()*100000000);
			...
			$(obj).prepend('<div id="dragwrapper'+randID+'"><div id="drag'+randID+'"><img width="8" height="56" alt="handle" src="site/js/p/handle.gif" title="Drag me left or right to see the before and after images" id="handle'+randID+'" /></div></div>'); // Create drag handle
			...
			$(obj).append('<img src="site/js/p/lt-small.png" width="7" height="15" id="lt-arrow'+randID+'"><img src="site/js/p/rt-small.png" width="7" height="15" id="rt-arrow'+randID+'">');

			if(o.showFullLinks)
			{	
				$(obj).after('<div class="balinks" id="links'+randID+'" style="position:relative"><span class="bflinks"><a class = "bfl" id="showleft'+randID+'" href="javascript:void(0)">ДО РЕМОНТА</a></span><span class="bflinks"><a class = "bfl" id="showright'+randID+'" href="javascript:void(0)">ПОСЛЕ РЕМОНТА</a></span></div>');
				$('#links'+randID).width(imgWidth);
				$('#showleft'+randID).css({'position':'relative','left':'18%'}).click(function(){
					$('div:eq(2)', obj).animate({width:imgWidth},200);
					$('#dragwrapper'+randID).animate({left:imgWidth-$('#dragwrapper'+randID).width()+'px'},200);
				});
				$('#showright'+randID).css({'position':'relative','left':'30%'}).click(function(){
